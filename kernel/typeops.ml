...
let should_invert_case env ind ci =
  ...
  (* NB: it's possible to have 2 ctors or arguments to 1 ctor by unsetting univ checks
     ...
     TODO consider more carefully what happens with letin params and arguments.
 *)
...
let type_of_case env ci p pt iv c ct _lf lft =
  ...
  let () =
    ...
    in
    assert (is_inversion = should_invert_case env (fst pind) ci)
  in
    ...
    | Case (ci,p,iv,c,lf) ->
        ...
        let iv' = match iv with
          ...
          | CaseInvert {univs;args} ->
            ...
            let args' = match kind ct' with App (_, a) -> a | Ind _ -> [| |] | _ -> assert false in
