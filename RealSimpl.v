...
Polymorphic Definition let_lazy {T : Type} {K : T -> Type} (e : T) (f : forall v : T, K v) : K e := f e.
Polymorphic Record Blocked (T : Type) := block { unblock : T }.
...
Fixpoint interpret_R (e : Expr_R) : R :=
  ...
  | ER_R r => (unblock r)
  | ER_Z z => delay (IZR z)
  | ER_Unary f a => delay (unary_fun_R f) (interpret_R a)
  | ER_Binary f a b => delay (binary_fun_R f) (interpret_R a) (interpret_R b)
...
Lemma cleanup_simplify_R_correct_new: forall (input output : Expr_R),
  cleanup_R (simplify_R input) = output ->
  interpret_R input = interpret_R output.
Proof. intros. rewrite <-H. apply cleanup_simplify_R_correct. Qed.
...
Ltac2 real_simplify_new (term : constr) (rhs : constr) : unit :=
  (dbg1 printf "real_simplify: term = %t" term);
  let ast := reify_Expr_R term in
  (dbg1 printf "real_simplify: AST = %t" ast);
  ...
  refine '(
    @let_lazy _
      (fun reduced : Expr_R =>
         interpret_R $ast = interpret_R reduced -> (* correctness lemma *)
         interpret_R reduced = $rhs ->             (* subgoal *)
         interpret_R $ast = $rhs                   (* original goal *)
      )
      (cleanup_R (simplify_R $ast))
      (
        fun (reduced : Expr_R)
            (correct : interpret_R $ast = interpret_R reduced)
            (subgoal : interpret_R reduced = $rhs)
        => eq_trans correct subgoal
      )
      (cleanup_simplify_R_correct $ast)
      $subgoal
  ).
...
Goal (x+(Rmax ((3*4+5^2-6)/7) (1/2)) = x + 31/7).
  lazy_match! goal with [ |- (?a = ?b) ] => real_simplify_new a b end.
  Unshelve.
  lazy_match! goal with [ |- ?g ] => Message.print (Message.of_constr g) end.
  reflexivity.
Qed.
